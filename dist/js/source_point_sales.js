 ...
 function agrega_a_lista(){
   $(document).ready(function(){
         if($("#cantidad").val()>0){
            ...
            $("#tabla_articulos > tbody").append("<tr><td class='center'>"+articulo+"</td><td class='center'>"+descripcion+"</td><td class='center' contenteditable>"+cantidad+"</td><td class='center'>"+precio+"</td><td class='center'>"+monto.toFixed(2)+"</td><td class='center'><button class='btn btn-block btn-danger btn-xs delete'><i class='icon-trash bigger-120'></i> Eliminar</button></td></tr>");
            $("#codigo").val("");
            $("#cantidad").val("");
            $("#preciou").val("");
            $("#cantidad").attr("disabled", true);
