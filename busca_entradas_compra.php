...
if($db->numero_de_registros($exe)>0){
 ...
 echo "<thead>";
 echo "<tr>";
 echo "<th>#Entrada</th><th>#Fecha</th><th>Proveedor</th><th>Monto</th><th>Descuento</th><th>IVA</th><th>Realizo</th><th>Estado</th><th>Detalles</th><th></th>";
 echo "</tr>";
 echo "</thead>";
 echo "<tbody>";
 while($ww=$db->buscar_array($exe)){
     ...
     where k.numero=$numero and k.proveedor=p.id");
      while($datas=$db->buscar_array($consulta_resumen)){
          ...
          $user=$datas['user'];
          $tipo= ($datas['tipo'] == "EC") ? "ACTIVA" : "CANCELADA" ;
          $fecha=$datas['fecha'];
      }
   echo "<tr>";
   ...
   echo "<td style='text-align: center;'><a href='#modal_tabla' data-toggle='modal'><button type='button' class='btn btn-mini btn-success' id=$entrada onclick='detalla_entrada(this.id);'><i class='fa fa-search'></i> Detalles</button></a></td>";
   if(preg_match('/(RECC)/',$permitions)){
    echo "<td style='text-align: center;'><button class ='btn btn-danger'  onclick='cancelaEntrada($entrada)'><i class='fa  fa-times'></i> Cancelar</button></td>";
   }
   echo "</tr>";
 }
 echo "</tbody>";
