...
$grupo=test_input($_POST['grupo']);
  ...
  if( $precio>=0){
    //if price is greater than 0 then check if there are a product with the same code 
    ...
    //check if exist the product register
    $revisa_articulo=$db->consulta("SELECT codigo FROM articulos WHERE codigo ='$codigo'");
    ...
    if($db->numero_de_registros($revisa_articulo) == 0){
      ...
      //"INSERT INTO articulos(codigo, descripcion, costo, precio, proveedor) values('$codigo','$descripcion',$costo,$precio,$proveedor)";
