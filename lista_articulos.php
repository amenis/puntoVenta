...
$cadena = "SELECT * FROM detallearticulos";
...
if($db->numero_de_registros($exe)>0){
 ...
 echo "<thead>";
 echo "<tr>";
 echo "<th>Codigo</th>/<th>Descripcion</th><th>Costo</th><th>Precio</th><th>Proveedor<th>Acciones</th>";
 echo "</tr>";
 echo "</thead>";
 echo "<tbody>";
 while($e=$db->buscar_array($exe)){
    ...
    if(preg_match('/(ARTM)/',$_SESSION['permisos'])){
      echo "<td style='text-align: center;'> <button class='btn btn-success' onclick='editar_articulo(\"".addslashes(strtoupper($e['codigo']))."\");' > <span class='glyphicon glyphicon-pencil'></span></button> </td>";
    }
